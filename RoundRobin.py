...
def do_process(list_final_process,list_process,current_time,ready_queue,t,current_and_t): # current_time is total time
    ...
    if(len(ready_queue)==0): # Ready queue is empty
        ...
        # If the process can be done in this time quantum
        if(current_process.current_cpu<=time_quantum):
            if(len(list_process)!=0):
                while(len(list_process)!=0 and t<len(list_process) and (list_process[t].current_arrival < (current_time+current_process.current_cpu) or (list_process[t].current_arrival == current_time+current_process.current_cpu and list_process[t].id<current_process.id))):
            ...
            ready_queue.pop(0) # The process is completely done, so it is removed from ready queue
            is_current_process_done = 1
            
            if(current_process.response<0):
                current_process.response = current_time - current_process.cpu1 - current_process.arrival
        
        
        # If the process is not done
        elif(current_process.current_cpu>time_quantum):
            if(len(list_process)!=0):
                while(len(list_process)!=0 and t<len(list_process) and (list_process[t].current_arrival < (current_time+time_quantum) or (list_process[t].current_arrival == current_time+time_quantum and list_process[t].id<current_process.id))):
                    ready_queue.append(list_process[t]) # Pushes the process to the queue
                    # Deletes element from the list of processes (list_process), beacause it wont need any process after that (io is done)
                    if(list_process[t].is_io_done==1):
                        del list_process[t]
            ...
            current_process.current_cpu -= time_quantum
            
            ready_queue.pop(0)
            ready_queue.append(current_process)
            
            if(current_process.response<0):
                current_process.response = current_time - time_quantum - current_process.arrival
        
    
    
    # Updates waiting time and turn around time and response time and current arrival
        if(current_process.is_io_done==0 and is_current_process_done==1):  
            ...
            if(len(list_process)>t): # There still exists at least one process that should be brought to the ready queue
                ...
                if(current_process.current_arrival < list_process[t].current_arrival):
                    ...
                    temp_t = t
                    while(temp_t<len(list_process) and current_process.id > list_process[temp_t].id and current_process.current_arrival == list_process[temp_t].current_arrival):
                        temp_t+=1
                    list_process.pop(0)
                    temp_t-=1
                    t-=1
                    ...
                    temp_t = t # The place that the current process should be placed in list_process after the io time
                    while(temp_t<len(list_process) and (current_process.current_arrival > list_process[temp_t].current_arrival or (current_process.id > list_process[temp_t].id and current_process.current_arrival == list_process[temp_t].current_arrival))):
                        temp_t+=1
                    list_process.pop(0)
                    temp_t-=1
                    t-=1
        ...
        elif(current_process.is_io_done==1 and is_current_process_done==1):
...
def round_robin(list_pro, num_pro):
    ...
    file = open("RoundRobin_Output.txt","w+")
    file.write(40*"=")
    file.write("\n")
    file.write(14*" ")
